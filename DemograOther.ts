...
async function CASSO_LOGIN_function() {
  await RPA.WebBrowser.get(Tableau_URL2);
...
async function Tabluea_Ope_function() {
  ...
  */
  //「AdxV4課金ステータス」をクリック
  const AdxV4KakinStatusButton = await RPA.WebBrowser.wait(
    RPA.WebBrowser.Until.elementLocated({
      className: 'tabComboBoxButtonHolder'
    }),
    30000
  );
  await AdxV4KakinStatusButton.click();
  await RPA.sleep(1000);
  ...
  await RPA.sleep(1000);
  //「日付」をクリック
  const DayButton = await RPA.WebBrowser.findElementsByClassName(
    'tabComboBoxButtonHolder'
  );
  await DayButton[4].click();
  await RPA.sleep(1000);
  //「すべて」にチェック
  All = await RPA.WebBrowser.findElementByClassName('FICheckRadio');
...
async function AdxV4Genre_function() {
  ...
  tabComboBoxButtonHolder`);
  await AdxV4GenreButton[5].click();
  await RPA.sleep(1000);
  //「すべて」をクリック
  All = await RPA.WebBrowser.findElementByClassName('FICheckRadio');
  await All.click();
  ...
  Check = await RPA.WebBrowser.findElementsByClassName('FICheckRadio');
  for (let i in CurrentGenre) {
    for (let n in AdxV4GenreList) {
      const Text = await AdxV4GenreList[n].getText();
      if (Text == CurrentGenre[i]) {
        if (CurrentGenre[i] == `恋リア2`) {
          const ArrowDown = await RPA.WebBrowser.findElementsByClassName(
            `FIItem`
          );
          await RPA.WebBrowser.sendKeys(ArrowDown[n], [
            RPA.WebBrowser.Key.ARROW_DOWN
          ]);
          await Check[n].click();
          RPA.Logger.info(Text + ': チェックしました');
          break;
...
async function Download_function() {
  ...
  await RPA.sleep(500);
  await RPA.WebBrowser.driver.executeScript(
    `document.getElementsByClassName('csvLink_summary')[0].click()`
  );
  RPA.Logger.info('待機中...');
